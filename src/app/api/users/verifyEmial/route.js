...
export const sendEmail = async ({ email, emailType, id }) => {
  try {
    ...
    const donorUser = await Donor.findOne({ email });
    ...
    if (!(user instanceof Organization || user instanceof Donor || user instanceof Patient)) {
    ...
    // Update the user based on its model
    if (emailType === 'VERIFY') {
